...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
    ...
    <aside class="fixed inset-y-0 left-0 z-20 w-64 bg-white shadow-xl border-r border-gray-100 flex-col hidden sm:flex">
        <div class="flex h-16 items-center justify-center border-b border-gray-200">
            <a href="#" class="flex items-center gap-2 font-bold text-lg bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
        ...
        <nav class="flex-1 p-4 space-y-2">
            <a href="index.php" class="flex items-center gap-3 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-3 py-2.5 font-semibold transition duration-200">
    ...
    <div class="flex flex-col sm:ml-64">
        ...
        <header class="sticky top-0 z-10 flex h-16 items-center justify-between bg-white border-b border-gray-200 px-6 shadow-sm">
            ...
            <div class="flex items-center gap-4">
                ...
                <img class="h-10 w-10 rounded-full object-cover border-2 border-blue-600" src="https://ui-avatars.com/api/?name=<?php echo urlencode($user_name); ?>&background=2563eb&color=fff" alt="User Avatar">
            </div>
        </header>
        ...
        <main class="flex-1 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                <div class="card-hover bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase">Total Siswa</p>
                        <p class="text-3xl font-bold text-gray-900"><?php echo $siswa_count; ?></p>
                    </div>
                </div>
                 <div class="card-hover bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 flex items-center gap-4">
                    <div class="p-4 bg-green-100 rounded-xl"><svg class="h-6 w-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase">Total Pembimbing</p>
                        <p class="text-3xl font-bold text-gray-900"><?php echo $pembimbing_count; ?></p>
                    </div>
                </div>
                <div class="card-hover bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase">Jurnal Pending</p>
                        <p class="text-3xl font-bold text-gray-900"><?php echo $jurnal_pending_count; ?></p>
                    </div>
                </div>
                <div class="card-hover bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 flex items-center gap-4">
                    <div class="p-4 bg-indigo-100 rounded-xl"><svg class="h-6 w-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase">Jurnal Disetujui</p>
                        <p class="text-3xl font-bold text-gray-900"><?php echo $jurnal_approved_count; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100">
                    <h3 class="font-bold text-gray-900 text-lg mb-5 flex items-center gap-2">
                        <svg class="w-5 h-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21a6 6 0 00-9-5.197M15 21a6 6 0 006-6v-1a6 6 0 00-9-5.197"/></svg>
                        Pengguna Baru
                    </h3>
                    <div class="space-y-3">
                        <?php foreach($recent_users as $user): ?>
                            <div class="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition">
                                <div class="flex items-center gap-3">
                                    <img class="h-10 w-10 rounded-full object-cover border-2 border-gray-200" src="https://ui-avatars.com/api/?name=<?php echo urlencode($user['nama_lengkap']); ?>&background=random&color=fff" alt="Avatar">
                                    <div>
                                        <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($user['nama_lengkap']); ?></p>
                                        <p class="text-xs text-gray-500"><?php echo htmlspecialchars($user['email']); ?></p>
                                    </div>
                                </div>
                                <span class="text-xs font-bold px-3 py-1 rounded-full <?php echo $user['role'] === 'siswa' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'; ?>">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100">
                    ...
                    <div class="space-y-3">
                         <?php foreach($recent_journals as $jurnal): ?>
                            <div class="flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:shadow-md transition">
                                <div class="flex items-center gap-3">
                                     <span class="p-2 rounded-lg <?php 
                                        if ($jurnal['status'] == 'approved') echo 'bg-green-100';
                                        elseif ($jurnal['status'] == 'rejected') echo 'bg-red-100';
                                        else echo 'bg-amber-100';
                                    ?>">
                                    ...
                                    <div>
                                        <p class="font-semibold text-gray-800 text-sm">Jurnal dari <span class="font-bold"><?php echo htmlspecialchars($jurnal['nama_siswa']); ?></span></p>
                                        <p class="text-xs text-gray-500 mt-1">
                                            ...
                                            ?>">
                                                ...
                                                ?>
                                            </span> · <?php echo date('d M Y', strtotime($jurnal['tanggal_kegiatan'])); ?>
                                        </p>
                                    </div>
                                </div>
                                <a href="#" class="text-sm font-bold text-blue-600 hover:text-blue-700 transition">Lihat →</a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
